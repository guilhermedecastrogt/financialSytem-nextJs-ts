...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  try {
    const category = await prisma.spentCategory.findUnique({
      where: {
        id: params.id,
      },
    })
    ...
    return NextResponse.json({ error: "Erro ao buscar categoria de despesa" }, { status: 500 })
  }
}
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const existingCategory = await prisma.spentCategory.findFirst({
      where: {
        slug,
        id: {
          not: params.id,
    ...
    return NextResponse.json({ error: "Erro ao atualizar categoria de despesa" }, { status: 500 })
  }
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const spentsUsingCategory = await prisma.spent.findMany({
      where: {
        categories: {
          some: {
            id: params.id,
    ...
    return NextResponse.json({ error: "Erro ao excluir categoria de despesa" }, { status: 500 })
  }
}
